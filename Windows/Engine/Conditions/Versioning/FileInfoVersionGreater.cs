...
{
    class FileInfoVersionGreater : AbstractCondition
    {
        ...
        public FileInfoVersionGreater(IEngine engine)
            : base (engine)
        {
        }
        ...
        public override BooleanReason Evaluate()
        {
            try
            {
                ...
                return version > compareVersion ?
                    new BooleanReason(true, "The stored version ({0}) is less than the target version ({1})", version, compareVersion) :
                    new BooleanReason(false, "The stored version ({0}) is greater than the target version ({1})", version, compareVersion);
            }
            catch (Exception e)
            {
        ...
        public override bool IsInitialized()
        {
            ...
            if (!IsPropertySet(CompareVersion))
                return false;
