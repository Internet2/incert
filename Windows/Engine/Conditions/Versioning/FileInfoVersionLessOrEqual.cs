...
{
    class FileInfoVersionLessOrEqual:AbstractCondition
    {
        ...
        public FileInfoVersionLessOrEqual(IEngine engine)
            : base (engine)
        {
        }
        ...
        public override BooleanReason Evaluate()
        {
            try
            {
                ...
                return version > compareVersion ? 
                    new BooleanReason(false, "The target version ({0}) is less than the stored version ({1})", version, compareVersion) : 
                    new BooleanReason(true, "The target version ({0}) is greater than or equal to the stored version ({1})", version, compareVersion);
            }
            catch (Exception e)
            {
