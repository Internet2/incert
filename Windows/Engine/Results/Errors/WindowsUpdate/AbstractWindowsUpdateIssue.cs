...
{
    abstract class AbstractWindowsUpdateIssue: ErrorResult
    {
        static internal string GetMessageForResult(int issue)
        {
            try
            {
                var message = ConvertResultToString((uint) issue);
                if (string.IsNullOrWhiteSpace(message))
                {
                    var error = Marshal.GetExceptionForHR(issue);
                    message = error == null ? "" : error.Message;
                }
              
                if (string.IsNullOrWhiteSpace(message))
                    message = string.Format("an unknown issue (0x{0:x}) has occurred", issue);

                return message;
            }
            catch (Exception)
            {
                return string.Format("an unknown issue (0x{0:x}) has occurred", issue);
            }
        }
        
        static internal string ConvertResultToString(uint issue)
        {
            switch (issue)
            {
                case 0x80240001:
